...
function MorphingSphere({ isReady }: { isReady: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < positionAttribute.count; i++) {
      ...
      const noise = Math.sin(vertex.x * 3 + time * 0.8) * 
                    Math.sin(vertex.y * 3 + time * 0.6) * 
                    Math.sin(vertex.z * 3 + time * 0.7);
      
      const displacement = 2 + noise * morphIntensity + 
...
// ============================================================================
function FloatingParticles({ count = 200, isReady }: { count?: number; isReady: boolean }) {
  ...
  const currentPositions = useRef(new Float32Array(particleData.initialPositions));
  ...
  useFrame((state, delta) => {
    ...
    if (isReady && expansionProgress.current < 1) {
      expansionProgress.current = Math.min(1, expansionProgress.current + delta * 0.25);  // Much slower expansion
    }
    ...
    currentSize.current = THREE.MathUtils.lerp(currentSize.current, targetSize, delta * 0.8);
    
    // Animate opacity - start at 0 and fade in
    const targetOpacity = shouldShow ? 0.8 : 0;
    currentOpacity.current = THREE.MathUtils.lerp(currentOpacity.current, targetOpacity, delta * 0.6);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const eased = particleProgress === 1 ? 1 : 1 - Math.pow(2, -10 * particleProgress);
    ...
    smoothedMouse.current.x = THREE.MathUtils.lerp(smoothedMouse.current.x, pointer.x, delta * 3);
    smoothedMouse.current.y = THREE.MathUtils.lerp(smoothedMouse.current.y, pointer.y, delta * 3);
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center">
      ...
      <div className="relative">
        {/* Pulsing outer ring */}
        <div className="absolute inset-0 -m-8 rounded-full border border-cyber-lime/20 animate-ping" style={{ animationDuration: '2s' }} />
        <div className="absolute inset-0 -m-16 rounded-full border border-electric-indigo/10 animate-ping" style={{ animationDuration: '3s', animationDelay: '0.5s' }} />
        
        {/* Main orb */}
        <div 
          className="w-32 h-32 rounded-full animate-pulse"
          style={{
            background: 'radial-gradient(circle, rgba(0,255,157,0.3) 0%, rgba(99,102,241,0.2) 50%, transparent 70%)',
          }}
        />
        
        {/* Floating dots */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-8">
          <div className="w-2 h-2 rounded-full bg-cyber-lime/50 animate-bounce" style={{ animationDelay: '0s' }} />
        </div>
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-8">
          <div className="w-2 h-2 rounded-full bg-electric-indigo/50 animate-bounce" style={{ animationDelay: '0.2s' }} />
        </div>
        <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-8">
          <div className="w-2 h-2 rounded-full bg-electric-purple/50 animate-bounce" style={{ animationDelay: '0.4s' }} />
        </div>
        <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-8">
          <div className="w-2 h-2 rounded-full bg-cyber-lime/50 animate-bounce" style={{ animationDelay: '0.6s' }} />
        </div>
      </div>
