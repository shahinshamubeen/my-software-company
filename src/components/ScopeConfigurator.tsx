...
export default function ScopeConfigurator() {
  ...
  const estimate = useMemo(() => {
    ...
    const activePlatforms = Object.entries(config.platforms).filter(([, v]) => v);
    if (activePlatforms.length > 0) {
      const avgMultiplier =
        activePlatforms.reduce((sum, [key]) => sum + PLATFORM_COSTS[key as keyof typeof PLATFORM_COSTS], 0) /
    ...
    const featureCost = Object.entries(config.features)
      .filter(([, v]) => v)
      .reduce((sum, [key]) => sum + FEATURE_COSTS[key as keyof typeof FEATURE_COSTS], 0);
          ...
          <div className="space-y-8">
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-3">
                Team Size
                <span className="ml-2 text-cyber-lime font-mono">{config.teamSize} developers</span>
              </label>
              <input
                type="range"
                min={1}
            ...
            <div>
              <label className="block text-sm font-medium text-white mb-3">
                Timeline
                <span className="ml-2 text-electric-indigo font-mono">{config.timeline} months</span>
              </label>
              <input
                type="range"
                min={1}
                ...
                onChange={(e) => setConfig({ ...config, timeline: parseInt(e.target.value) })}
                      ...
                      <span className="flex items-center gap-2">
                        <span
                          className={`w-4 h-4 rounded border flex items-center justify-center ${
                            value ? 'bg-electric-purple border-electric-purple' : 'border-white/30'
                          }`}
                        >
                          {value && (
                            <svg
            ...
            <div className="flex-1 p-6 rounded-xl bg-white/5 border border-white/10">
              ...
              <div className="mt-6 space-y-2 text-sm">
                <div className="flex justify-between text-white/60">
                  <span>Team</span>
                  <span className="font-mono">{config.teamSize} developers</span>
                </div>
                <div className="flex justify-between text-white/60">
                  <span>Duration</span>
